...
radical			PROC
				...
				; r0 = x
				MOV r1, #2		; index
				EOR r2, r2		; used to know if the index was already multiplied
				MOV r3, #1		; result
cycleRadical	SDIV r4, r0, r1		; divide by the index
				MUL r4, r4, r1		; multiply by the index, in this way we can know if it is a divisor
				CMP r4, r0
				BEQ isDivisor
					; isntDivisor
					ADD r1, #1	; next index
					EOR r2, r2	; reset the flag
					B cycleRadical
isDivisor		CMP r2, #0
				BNE alreadyCounted
					; multiply the index
...
alreadyCounted	SDIV r0, r0, r1		; divide for the next iteration
				CMP r0, #1		
				BNE cycleRadical	; loop untill x = 1
				MOV r0, r3		; for return
				POP {r4,PC}
				ENDP	
